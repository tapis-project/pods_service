...
- **Runtime on Python 3.12** — base image bumped from python:3.10 (EOL October 2026); CI gates run 3.12 to match. Not 3.13: that release removed `lib2to3`, which `tapisservice` still imports, so the service cannot start on it until the SDK drops that import.
- **Dependency refresh to current** — fastapi 0.141 (starlette 1.3), kubernetes client 36, which unlocks urllib3 2.7 and brings tapipy from 1.9 to 25.4 (the old kubernetes pin's urllib3 cap had been silently holding the Tapis SDK two years back). pip-audit now reports zero known vulnerabilities. psycopg2 bumped to 2.9.12, which also makes the requirements installable on Python 3.13.

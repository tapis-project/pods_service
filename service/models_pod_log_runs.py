...
class PodLogRunBase(TapisApiModel):
    ...
    logs: Optional[str] = Field(default=None, sa_type=Text, description="Pod stdout for this run.")
...
class PodLogRun(TapisPodLogRunBaseFull, table=True, validate=True):
    ...
    # Indexes are declared here as well as in the migrations that created them —
    # `alembic check` diffs models against the DB, so anything only the migration
    # knows about shows up as drift (and autorevision would offer to DROP it).
    __table_args__ = (
        UniqueConstraint("pod_id", "tenant_id", "run_index", name="uq_pod_log_run"),
        Index("ix_pod_log_runs_pod_tenant_idx", "pod_id", "tenant_id", "run_index"),
        Index("ix_pod_log_runs_pod_tenant_active", "pod_id", "tenant_id", "is_active"),
    )
